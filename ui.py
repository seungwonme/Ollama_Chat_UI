...
from llm import generate_title, generate_ai_response
...
def update_messages(session_id, prompt, llm):
    # 사용자 메시지
    ...
    # AI 응답
    full_text = generate_ai_response(llm, st.session_state["messages"])
    st.session_state["messages"].append(AIMessage(content=full_text))
    save_message(session_id, "ai", full_text)

    # 메시지가 정확히 2개(첫 대화)일 때 제목 자동 생성
    if len(st.session_state["messages"]) == 2:
        title = generate_title(st.session_state["messages"], llm)
